...
func editorOpen() {
	...
	for {
		...
		bite := byte(E.password[nn%(len(E.password))])
		writebuf := make([]byte, 0)
		writebuf = append(writebuf, bite)
		xord, err := xor.XORBytes(writebuf, buf[0:n])
		bufw = append(bufw, xord[0])
	}

	//bufwr := strings.NewReader(string(bufw))
	//fp_buf := bufio.NewReader(bufwr)

	for _, line := range strings.Split(string(bufw), "\n") {
		//for line, err := bufw.ReadBytes('\n'); err == nil; line, err = fp.ReadBytes('\n') {
...
func editorSave() {
	...
	for {
		...
		writebuf := make([]byte, 0)
		writebuf = append(writebuf, bite)
		xord, err := xor.XORBytes(writebuf, bufw[0:n])
		bufwa = append(bufwa, xord[0])
	}

	n, err := io.WriteString(fp, string(bufwa))
	if err == nil {
		if n == length {
			E.dirty = false
			editorSetStatusMessage("%d bytes written to disk", length)
		} else {
			editorSetStatusMessage(fmt.Sprintf("wanted to write %d bytes to file, wrote %d", length, n))
		}
		return
	}
...
func start_editor(filename string, password string) {
	E.password = password
	E.filename = filename
	enableRawMode()
...
func launch_kilo(filename string, password string) {
	start_editor(filename, password)
